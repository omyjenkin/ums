...
namespace UMS.Framework.Web
{
    public class WebControllerBase : Controller
    {
        ...
        /// JS关闭弹窗，刷新浏览器整个页面
        /// </summary>
        /// <returns>使用JavaScriptResult而不是ContentResult，是因为页面中使用了ajaxSubmit提交</returns>
        public JavaScriptResult RefreshParentJS(string param = null, string alert = null)
        {
            var script = string.Format("{0}; parent.location.href=parent.location.href; top.tb_remove();", string.IsNullOrEmpty(alert) ? string.Empty : "alert('" + alert + "')");
            //var script = string.Format("{0};var b = parent.location.href.lastIndexOf('?');if (b<0) parent.location.href=parent.location.href+\"{1}\";else parent.location.href=parent.location.href+\"{2}\"; top.tb_remove();", string.IsNullOrEmpty(alert) ? string.Empty : "alert('" + alert + "')", param, param.Replace("?", "&"));
        ...
        /// </summary>
        /// <returns></returns>
        public ContentResult RefreshParentContent(string param = null, string alert = null)
        {
            var script = string.Format("<script>{0};parent.location.href=parent.location.href;top.tb_remove();</script>", string.IsNullOrEmpty(alert) ? string.Empty : "alert('" + alert + "')");
