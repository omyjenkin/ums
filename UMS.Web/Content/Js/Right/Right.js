$(function () {
    ...
    $('#roleList').datagrid({
        ...
        columns: [[
            ...
            { field: 'CreateUser', title: '创建人', width: 80, sortable: true, hidden: true }
        ]],
        onClickRow: function (index, data) {
            ...
            if (row != null) {
                ...
                curRoleId = row.Id;
                $('#operateList').datagrid({ url: "/Right/GetRightByRoleAndModule?roleId=" + curRoleId + "&moduleId=" + curModuleId + "" });
                $('#operateList').datagrid({ 'title': "角色组: " + curRoleName + " >> 模块：" + curModuleName });
            }
        }
    });
    $('#moduleList').treegrid({
        url: '/Right/GetModelList',
        width: 300,
        method: 'post',
        height: $(window).height() - 35,
        fitColumns: true,
    ...
    $('#operateList').datagrid({
        url: '/Right/GetRightByRoleAndModule',
    ...
    $("#btnSave").click(function () {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            {
                $.post('/Right/UpdateRight', { "Id": rows[i].Ids, "RightId": rows[i].RightId, "KeyCode": rows[i].KeyCode, "IsValid": setFlag }, "json");
                updateRows++;
            }
        }
        if (updateRows > 0) {
