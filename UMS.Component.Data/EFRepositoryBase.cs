...
namespace UMS.Component.Data
{
    ...
    public abstract class EFRepositoryBase<TEntity, TKey> :  IRepository<TEntity, TKey> where TEntity : class,new()
    {
        #region 属性
        ...
        /// <returns></returns>
        public virtual IQueryable<TEntity> GetQueryable(Expression<Func<TEntity, bool>> match, string sortPropertyName, bool isDescending = true)
        {
            ...
            var keySelector = Expression.Lambda<Func<TEntity,TKey>>(body, param);
        ...
        public virtual async Task<int> InsertAsync(TEntity entity)
        {
            PublicHelper.CheckArgument(entity, "entity");
            EFContext.RegisterNew<TEntity, TKey>(entity);
            return await EFContext.CommitAsync();
        }
        ...
        public virtual async Task<int> DeleteByConditionAsync(Expression<Func<TEntity, bool>> match)
        {
            PublicHelper.CheckArgument(Entities, "entities");
            EFContext.RegisterDeleted<TEntity, TKey>(Entities.Where<TEntity>(match).ToList<TEntity>());
        ...
        {
           return EFContext.DbContext.Database.ExecuteSqlCommand(TransactionalBehavior.EnsureTransaction,sql, parameters);
        }
        ...
        /// <param name="sql"></param>
        /// <returns></returns>
        public virtual IQueryable<T> QuerySql<T>(string sql, params object[] parameters)
        {
            return EFContext.DbContext.Database.SqlQuery<T>( sql, parameters).AsQueryable();
        }
        ...
        /// <param name="sql"></param>
        /// <returns></returns>
        public virtual IQueryable<dynamic> DynamicQuery<dynamic>(string sql, params object[] parameters)
        {
            return EFContext.DbContext.Database.SqlQuery<dynamic>(sql, parameters).AsQueryable();
        }
